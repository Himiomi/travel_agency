...
public record SiteService(PredictionEngineClient predictionEngineClient) {
    public ArrayList<PotentialDestinations> potentialCountries(Weather meteo, int minTemp) throws IOException {
        ...
        for (Iterator<String> it = content.lines().iterator(); it.hasNext(); ) {
            ...
            try {
                ...
                if (moyTemp > minTemp && meteo.equals(Weather.WARMER))listDest.add(new PotentialDestinations(country, moyTemp));
                else if (moyTemp < minTemp && meteo.equals(Weather.COLDER))listDest.add(new PotentialDestinations(country, moyTemp));
